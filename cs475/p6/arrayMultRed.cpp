...
const char *			CL_FILE_NAME = { "arrayMultRed.cl" };
...
main( int argc, char *argv[ ] )
{
	...
	status = clSetKernelArg( kernel, 2, sizeof(cl_mem), NULL ); // local "prods" array - one per work item.
	if( status != CL_SUCCESS )
	...
	fprintf( stderr, "%8d, %4d, %10d, %8d, %10.3lf GigaMultsReductionPerSecond\n",
		NMB, LOCAL_SIZE, NUM_WORK_GROUPS, NUM_ELEMENTS, (double)NUM_ELEMENTS/(time1-time0)/1000000000. );

	fprintf( stdout, "%8d, %4d, %10d, %8d, %10.3lf\n",
		NMB, LOCAL_SIZE, NUM_WORK_GROUPS, NUM_ELEMENTS, (double)NUM_ELEMENTS/(time1-time0)/1000000000. );
