...
void dynarray_insert(struct dynarray* da, void* val) {
  if(da->size == da->capacity){
    da->data = realloc(da->data, 2 * da->capacity * sizeof(void*));
    da->capacity = 2 * da->capacity;
    da->data[da->size] = val;
    da->size++;
    /*int n = da->size;
    struct dynarray* da2 = malloc(2 * (sizeof(da)));
    strcpy(da2, da->data);*/
  }
  else{
    //printf("da size = %i\n", da->size);
  ...
  //printf("da data = %s", da->data);
  //printf("          DA SIZE = %i", da->size);
  //printf("    DA CAPACITY = %i\n", da->capacity);
