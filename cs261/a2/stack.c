...
void* stack_pop(struct stack* stack) {
  //void* remove = stack_top(stack->list);
  //void* top = get_top(stack->list);

  void* top = stack_top(stack);
  struct link* temp = get_head(stack->list);
  struct list* next = get_next(stack->list);
  stack->list = next;
