...
char* encrypt(char* msg, char* key){
  ...
  char *encryptedMsg = malloc(sizeof(char)* msgLength), afterEncrypt[strlen(msg)-1];
  ...
  while(i < msgLength - 1){
    if(msg[i] == ' ' && key[i] == ' '){
      afterEncrypt[i] = (bank[((msg[i] - 6) + (key[i] - 6)) %27]);
    }
    else if(msg[i] == ' '){
      afterEncrypt[i] = (bank[((msg[i] - 6) + (key[i] - 65)) %27]);
    }
    else if(key[i] == ' '){
      afterEncrypt[i] = (bank[((msg[i] - 65) + (key[i] - 6)) %27]);
    }
    else{
      afterEncrypt[i] = (bank[((msg[i] - 65) + (key[i] - 65)) %27]);
    }
    i++;
  }

  afterEncrypt[msgLength - 1] = '\n';
  afterEncrypt[msgLength] = '\0';
  strcpy(encryptedMsg, afterEncrypt);
  //printf("ENCRYPTED == %s\n", encryptedMsg);
...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    if(pid == 0) {
      ...
      //SENDING ENCRYPTED TEXT TO CLIENT
			char* finalenc = encrypt(msg, key);
			//printf("FINALENC == %s\n", finalenc);
			charsRead = write(connectionSocket, finalenc, strlen(finalenc));
			if(charsRead < 0){
        error("ERROR reading from socket");
      }
